...
<style>
    ...
    .st-gamma { background-color: #6c5ce7; color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; }
    .st-squeeze { background-color: #0984e3; color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; }
    .st-value { background-color: #00b894; color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; }
    .st-dip { background-color: #e17055; color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; }
    ...
    def fetch_single(ticker):
        try:
            ...
            if cur > upper_bb.iloc[-1] and vol_ratio > 1.8:
                category = "SHORT"; strat_name = "🚀 수급 돌파"; strat_class = "st-gamma"
                desc = f"거래량 폭발({vol_ratio:.1f}배) + 밴드 상단 돌파"
                target_pct, stop_pct, trail_pct, time_stop_days = 0.12, 0.05, 0.03, 2
            elif sc_squeeze > 8.0:
                category = "SWING"; strat_name = "🌊 에너지 응축"; strat_class = "st-squeeze"
                desc = "변동성 극소화, 시세 분출 임박"
                target_pct, stop_pct, trail_pct, time_stop_days = 0.15, 0.05, 0.04, 10
                ...
                target_pct, stop_pct, trail_pct, time_stop_days = 0.08, 0.07, 0.03, 5
                ...
                target_pct, stop_pct, trail_pct, time_stop_days = 0.30, 0.10, 0.10, 60
            ...
            multiplier = 1.2 if sc_squeeze > 8.0 else 1.0
            if category == "NONE": multiplier = 0.0
            bet_text = "비중:최대" if multiplier >= 1.2 else "비중:보통" if multiplier >= 1.0 else "비중:축소" if multiplier > 0 else "관망"
