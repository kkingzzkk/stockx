...
# === [1. 페이지 설정] ===
st.set_page_config(page_title="QUANT NEXUS : MASTER UI", page_icon="🦅", layout="wide", initial_sidebar_state="expanded")
...
<style>
    ...
    .act-buy { border: 2px solid #00FF00 !important; } 
...
def get_market_data(tickers, effective_nav, consec_loss):
    ...
    def fetch_single(ticker):
        try:
            ...
            if category == "SCALP": target_price = target_calc 
            else: target_price = min(call_wall, target_calc) if (has_option and call_wall > cur) else target_calc
            ...
            return {
                ...
                "BetAmount": qty * cur, "BetText": bet_text, "Score": score, "Action": action_status,
...
if target_tickers:
    ...
    if not market_data:
        st.warning("🔍 조건에 맞는 종목이 없습니다. (시장 침체 또는 데이터 부족)")
        st.info("💡 팁: 전체 시장이 하락세이거나, 초우량주 위주로 수급이 몰린 경우 신호가 적을 수 있습니다.")
    else:
        if mode == "🏆 AI 전체 시장 스캔" and 'scan_option' in locals() and scan_option == "💎 AI 추천 TOP 50":
            ...
            st.success(f"💎 AI 점수 상위 {len(market_data)}개 종목을 추출했습니다.")
